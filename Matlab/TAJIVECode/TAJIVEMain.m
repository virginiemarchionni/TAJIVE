function outstruct = TAJIVEMain(datablock, vecr, nComm, paramstruct)
...
%                    n is along the rows
%
%   vecr             -  a matrix of estimated signal ranks for each factor
%                       matrix of each data block.
%                   colums identifies data_block
%                    A needed input where no default is provided;
%                    recommend to use 'TAJIVEPreVisualMJ.m' for selection
%   nComm            -  number of common dimensions.
%
...
%             rankv{ib} = vecr(ib);
%         end
%         TAJIVEPreVisualMJ(datablock, rankv, dataname, numcompshow,  iprint(1), figdir);
%     end

for iC=1:nComm
    fprintf('Analysis of mode %d\n',iC);
    fprintf('~~~~~~~~~~~~~~~~~~~~\n');

    % Step 1: Signal Space Initial Extraction
    fprintf('Signal Space Initial Extraction ... \n')
    [M, angleBound, threshold] = TAJIVEInitExtract(datablock, vecr, iC, nresample, dataname, ierror0);
    
    
    % Step 2: Joint Score Space Estimation
    fprintf('Score Space Segmentation ... \n')
    row_joint_origin = TAJIVEJointSelect(M, angleBound, vecr, iC, threp, rj, dataname, ...
    ...
    outstruct{iC} = TAJIVEReconstruct(datablock,iC, threshold, dataname, row_joint_origin, ioutput, ferror);
